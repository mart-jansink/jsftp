...
var Ftp = module.exports = function(cfg) {
  ...
  // To keep backwards compatibility: split the timeout option in a number for
  // the initial connection timeout and one used for the PASVsockets later on.
  if (!this.timeout || (typeof this.timeout === 'number')) {
    this.timeout = {
      initial: 0,
      default: 0,
      pasv: this.timeout || TIMEOUT
    }
  }
  ...
  // Don't connect straight away anymore, instead wait for the first command to
  // be given that'll call _createSocket if required. 
};
...
Ftp.prototype._createSocket = function(port, host, firstAction) {
  ...
  this.socket.setTimeout(self.timeout.initial);
  this.socket.on('connect', function(data) {
    self.reemit('connect')(data);
    // Allow the connection to be more lenient with a timeout after it has
    // actually connected.
    self.socket.setTimeout(self.timeout.default);
  });
  this.socket.on('timeout', function() {
    self.destroy()
    // Treat an initial timeout as an error.
    self.reemit('error')(new Error('connect ETIMEOUT'));
  });
...
Ftp.prototype.parseResponse = function(response) {
  ...
  var next = this.commandQueue[0];
  if (response.isMark) {
    // If we receive a Mark and it is not expected, we ignore that command
    if (!next.expectedMarks ||
        next.expectedMarks.marks.indexOf(response.code) === -1) {
      return;
    }

    // We might have to ignore the command that comes after the mark.
    if (next.expectedMarks.ignore) {
      this.ignoreCmdCode = next.expectedMarks.ignore;
    }
  }
 ...
 */
Ftp.prototype.execute = function(action, callback, expectedMarks) {
  if (this.socket && this.socket.writable) {
    return this.runCommand(action, callback || NOOP, expectedMarks);
  }
...
Ftp.prototype.runCommand = function(action, callback, expectedMarks) {
  var cmd = {
    action: action,
    callback: callback,
...
Ftp.prototype.list = function(path, callback) {
  ...
  callback = once(callback);
    ...
    self.getPasvSocket(function(err, socket) {
      ...
      self.execute('list ' + (path || ''), cmdCallback, expectedMarks);
    });
};
...
Ftp.prototype.getGetSocket = function(path, callback) {
  ...
  this.getPasvSocket(function(err, socket) {
    ...
    self.execute('retr ' + path, cmdCallback, expectedMarks);
  });
};
...
Ftp.prototype.getPutSocket = function(path, callback, doneCallback) {
  ...
  this.getPasvSocket(function(err, socket) {
    ...
    self.execute('stor ' + path, putCallback, expectedMarks);
  });
};
...
Ftp.prototype.getPasvSocket = function(callback) {
  ...
  this.execute('pasv', function(err, res) {
    ...
    getPasvPort(res.text, function(err, options) {
      ...
      socket.setTimeout(self.timeout.pasv);
      self.socket.setTimeout(self.timeout.pasv);
      socket.once('connect', function() {
        self._pasvSocket = socket;
      });
      socket.once('close', function() {
        self._pasvSocket = undefined;
        self.socket && self.socket.setTimeout(self.timeout.default);
      });
